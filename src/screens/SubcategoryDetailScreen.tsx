...
export const SubcategoryDetailScreen = ({ route, navigation }: Props) => {
  ...
  const isUserCancellation = (error: any): boolean => {
    ...
    return (
      ...
      errorString.includes('user did not share') ||
  ...
  const handleDownloadPDF = () => {
    try {
      // Build query parameters based on current filter selection
      ...
      // Add payment_status filter if user is not a management viewer (only show success)
      // or if there's a specific filter applied
      if (!isManagementViewer) {
        params.append('payment_status', 'success');
      } else if (sortedDonations.length > 0) {
        ...
        if (allSameStatus && firstStatus !== 'success') {
          params.append('payment_status', firstStatus);
        }
      }

      // Build the full URL - using /pdf endpoint as per reference
      const url = `${BASE_URL}/api/donations/summary/pdf?${params.toString()}`;
  ...
  // Check if user can share PDF (ADMIN, SUB_ADMIN, or DONATION_MANAGER mapped to this subcategory)
  const canSharePDF = useMemo(() => {
    if (!currentUser) return false;
    
    // ADMIN and SUB_ADMIN can always share
    if (currentUser.role && currentUser.role.some(r => ['ADMIN', 'SUB_ADMIN'].includes(r))) {
      return true;
    }
    
    // DONATION_MANAGER can share only if mapped to this subcategory
    if (currentUser.role && currentUser.role.includes('DONATION_MANAGER')) {
      if (!currentUser.id) {
        return false;
      }
  ...
  const floatingActions = useMemo(() => {
    return [
      ...
      },
      {
        ...
        visible: canSharePDF && sortedDonations.length > 0,
      },
      {
        key: 'map',
        label: 'Map manager',
  ...
  }, [canManageSubcategory, canSharePDF, isSubcategoryActive, sortedDonations.length]);
